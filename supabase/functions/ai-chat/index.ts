...
serve(async (req) => {
  ...
  try {
    ...
    // Apply SBU filter if provided (filter is named "regionFilter" for backwards compat,
    // but matches against account_sbu)
    let allOpps = opps || [];
    const activeRegion = regionFilter && regionFilter !== 'all' ? String(regionFilter).toLowerCase() : null;
    if (activeRegion) {
      allOpps = allOpps.filter(o => (o.account_sbu || '').toString().trim().toLowerCase() === activeRegion);
    }
